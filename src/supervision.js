...
export function reviewMessage(binding, agent, reason) {
  ...
  return `Worker review · ${binding.agentSession.agent} ${binding.paneId}\n\nThis turn concerns only goal ${binding.goalId || "(local)"}. Use relevant earlier supervisor history when helpful, but do not carry another worker's facts into this decision.\n\nGoal\n  ${binding.goal}\n\nRequired context\n${context}\n\nConstraints\n${constraints}\n\nCurrent progress\n  ${progress}\n\nWhy review now\n  ${reason}; Herdr reports ${agent?.agent_status || "missing"}.\n\nWorker acceptance criteria\n${criteria}\n\nReview\n  Observe this exact worker once, then call exactly one decision tool. Your own response is not worker evidence and cannot satisfy these criteria.`;
}
