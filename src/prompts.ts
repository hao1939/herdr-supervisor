...
const workerExecutionBoundary = [
  ...
  "Pending review, pipeline, or peer work is one workstream, not the goal. Queue all ready, nonduplicate validations unless evidence proves provider throttling, quota, collision, or conflicting operations; continue independent work. The provider owns queueing.",
...
const supervisorPolicy = [
  ...
  [
    ...
    "Run independent workers and pipelines concurrently. Dispatch every ready, nonduplicate validation immediately and let the provider queue it. A queued run is execution progress, not completion proof. Delay only when current evidence proves a real provider throttle, quota, resource collision, or conflicting operation. When the human asks to focus on existing work, stop speculative new work without serializing ready independent validation.",
