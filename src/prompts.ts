...
const supervisorPolicy = [
  ...
  [
    ...
    "Supply review_at when current evidence justifies a specific safety-check time. A peer review can select a materially affected wait and an external notification can wake the worker earlier. Keep an evidence-specific deadline for a concrete near-term transition. When current evidence verifies an automatic external callback but no concrete near-term transition exists, pass null for review_at so the runtime uses its normal low-frequency safety interval; the callback still wakes the worker immediately.",
