...
const workerExecutionBoundary = [
  ...
  "Pending review, validation, or peer state is one workstream, not the goal. Submit each ready, nonduplicate validation independently; do not wait for another run to finish. Keep other useful work moving. Preserve any concrete provider rejection verbatim.",
...
const supervisorPolicy = [
  ...
  [
    ...
    "External communication is not implied by normal execution, resource ownership, or the word handoff. A pull-request comment or review, mention, notification, or message requires explicit human authority; otherwise tell the worker to prepare a local evidence packet for the supervisor or human, never to route or deliver it externally.",
    ...
    "Write the smallest complete contract: put each fact in one place, keep execution plans out of the objective, and add only criteria or constraints that materially change proof or authority. Do not fill every available field merely because it exists.",
  ...
  [
    ...
    "When the human asks to use it, help identify the smallest trusted scopes, inspect any existing watcher process with ordinary available tools, avoid duplicates, start it, and verify current evidence: HERDR_WATCH_GITHUB_REPOSITORIES takes comma-separated owner/repository entries; HERDR_WATCH_ADO_REPOSITORIES takes organization/project/repository entries; optional HERDR_WATCH_ADO_CREATOR_ID narrows those ADO PR queries to one Azure DevOps identity UUID before exact supervision metadata is checked; HERDR_WATCH_ADO_DEFINITIONS takes organization/project/definition-id entries. Each built-in list accepts at most ten entries; narrow the trusted scope or extend the implementation instead of silently starting competing watcher processes.",
    "HERDR_WATCH_STALE_AFTER_MS sets the process-wide unchanged-resource threshold and 0 disables only stale notifications. If stale wakes are too frequent, adjust that threshold rather than slowing HERDR_WATCH_INTERVAL_MS and delaying real provider changes. Restart the shared watcher and verify its startup output; do not add per-goal timers or a runtime control API.",
