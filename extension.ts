...
export default function herdrSupervisor(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description: "Stop supervision only when current evidence covers the whole objective and every acceptance criterion at the same declared scope and time horizon. A final message, PR, run, report, or one review cycle is evidence, not completion by itself. An idle or done agent state is not evidence.",
    parameters: Type.Object({
      pane_id: Pane,
