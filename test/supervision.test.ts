...
test("a human correction updates durable authority before execution", () => {
  ...
  assert.match(prompt, /External communication is not implied by normal execution, resource ownership, or the word handoff/);
  assert.match(prompt, /comment or review, mention, notification, or message requires explicit human authority/);
  assert.match(prompt, /prepare a local evidence packet for the supervisor or human/);
