...
condition it was waiting for. The summary is checkpointed with that revision so
a busy worker receives the same context after it settles or the process
restarts. The worker rereads provider authority and judges the change. When the
worker settles, its ordinary Herdr event asks the supervisor to review the
result.
...
If the exact worker becomes busy during that write, the prior wait and
not-yet-attempted state are restored so delivery can retry after settlement;
successful delivery records the exact post-delivery boundary. If delivery or
identity cannot be confirmed, the unresolved change remains durable and the
